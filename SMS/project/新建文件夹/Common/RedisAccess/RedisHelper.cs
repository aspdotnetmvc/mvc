...
namespace RedisAccess
{
    public class RedisHelper
    {
        ...
        private static PooledRedisClientManager CreateManager(string[] readWriteHosts, string[] readOnlyHosts)
        {
            ...
            {
                MaxWritePoolSize = Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["RedisMaxWritePoolSize"]), // “写”链接池链接数 
                MaxReadPoolSize = Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["RedisReadPoolSize"]), // “读”链接池链接数 
                AutoStart = true,
            });
        }
        #endregion
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        public static List<T> Hash_GetValueAll<T>(string key)
        {
            using (IRedisClient redis = prcm.GetClient())
            {
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        public static List<KeyValuePair<string, T>> Hash_GetAll<T>(string key)
        {
            List<KeyValuePair<string, T>> rt = new List<KeyValuePair<string, T>>();
            using (IRedisClient redis = prcm.GetClient())
            {
                var listKey =redis.GetHashKeys(key);
                var list = redis.GetHashValues(key);
                if (list != null && list.Count > 0)
                {
                    for(int i=0;i<listKey.Count;i++)
                    {
                        var value = ServiceStack.Text.JsonSerializer.DeserializeFromString<T>(list[i]);
                        rt.Add(new KeyValuePair<string, T>(listKey[i], value));
                    }
                }
                return rt;
            }
