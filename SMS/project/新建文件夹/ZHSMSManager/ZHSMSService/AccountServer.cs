    ...
    public class AccountServer
    {
        ...
        internal List<SMS.Model.EnterpriseUser> GetAccounts()
        {
            ...
            if (list == null)
            {
                ...
                try
                {
                    list = DAL.EnterpriseUser.GetEnterprises();
                }
                catch
                {
        ...
        internal bool DelAccount(string account)
        {
            try
            {
                bool ok = DAL.EnterpriseUser.Del(account);
                if (ok)
                {
        ...
        /// </summary>
        /// <returns></returns>
        internal List<SMS.Model.EnterpriseUser> GetAuditEnterprise()
        {
        ...
        /// </summary>
        /// <returns></returns>
        internal List<SMS.Model.AuditEnterprise> GetUnableEnterprises()
        {
        ...
        /// <param name="user"></param>
        /// <returns></returns>
        internal bool AddAuditEnterprise(SMS.Model.AuditEnterprise user)
        {
            try
            {
                bool ok = DAL.AuditEnterprise.Add(user);
                if (ok)
                {
                    dic.Add(user.EnterpriseCode, user);
                    return true;
                }
            }
            catch
        ...
        /// <param name="user"></param>
        /// <returns></returns>
        internal bool UpdateAuditEnterprise(SMS.Model.AuditEnterprise user)
        {
            try
            {
                bool ok = DAL.AuditEnterprise.Update(user);
                if (ok)
                {
                    if (user.EnterpriseResult)
                    {
                        dic.Remove(user.EnterpriseCode);
                        return true;
                    }
                    else
                    {
                        if (dic.ContainsKey(user.EnterpriseCode))
                        {
                            SMS.Model.AuditEnterprise user1 = dic[user.EnterpriseCode];
                            user.CreateTime = user1.CreateTime;
                            user.EnterpriseName = user1.EnterpriseName;
                            dic[user.EnterpriseCode] = user;
                            return true;
                        }
                    }
                }
        ...
        /// <returns></returns>
        internal bool DelAuditEnterprise(string enterprise)
        {
            try
            {
                if (dic.ContainsKey(enterprise))
                {
                    bool ok = DAL.AuditEnterprise.Delete(enterprise);
                    if (ok)
                    {
                        dic.Remove(enterprise);
                        return true;
                    }
                    return false;
                }
        ...
        internal bool AddSMSTempletContent(SMSTemplet content)
        {
            ...
            try
            {
                ok = DAL.SMSTempletDB.Add(content);
                if (ok)
                {
        ...
        internal bool UpdateAuditSMSTemplet(SMSTemplet content)
        {
            ...
            try
            {
                ok = DAL.SMSTempletDB.Update(content);
                if (ok)
                {
                    CacheManager<SMSTemplet>.Instance.Set(content.TempletID, content);
                }
        ...
        internal bool DelSMSTemplet(string templetID)
        {
            ...
            try
            {
                ok = DAL.SMSTempletDB.Del(templetID);
                if (ok)
                {
        ...
        internal bool UpdateSMSTemplet(SMSTemplet content)
        {
            ...
            try
            {
                ok = DAL.SMSTempletDB.UpdateContent(content);
                if (ok)
                {
                    CacheManager<SMSTemplet>.Instance.Set(content.TempletID, content);
                }
        ...
        internal SMSTemplet GetSMSTemplet(string templetID)
        {
            ...
            if (content == null)
            {
                ...
                try
                {
                    content = DAL.SMSTempletDB.GetSMSTempetContent(templetID);
                    if (content != null)
                    {
                        CacheManager<SMSTemplet>.Instance.Set(content.TempletID, content);
                    }
        ...
        {
            List<SMS.Model.EnterpriseUser> list = DAL.EnterpriseUser.GetEnterprises();
            ...
            List<SMS.Model.AuditEnterprise> li = DAL.AuditEnterprise.GetAuditOrFailureEnterprise();
            foreach (var v in li)
            {
                MessageTools.MessageHelper.Instance.WirteTest(v.EnterpriseCode);
                if (!dic.ContainsKey(v.EnterpriseCode))
                {
                    dic.Add(v.EnterpriseCode, v);
                }
            }

            List<SMSTemplet> sc = DAL.SMSTempletDB.GetSMSTempets();
            foreach (var v in sc)
            {
