...
namespace SMSService
{
    public partial class SMSService : MarshalByRefObject, ISMSService
    {
        ...
        public RPC_Result<string> GetSMSNumbersBySMSID(string SMSID)
        {
            ...
            try
            {
                var r = SMS.DB.SMSDAL.GetNumbersBySMSID(SMSID);
                return new RPC_Result<string>(true, r, "");
            }

            catch (Exception ex)
            {
                MessageTools.MessageHelper.Instance.WirteError("查询短信号码时发生了异常", ex);
                return new RPC_Result<string>(false, null, "查询短信号码时发生了异常");
            }
        }
        #endregion
